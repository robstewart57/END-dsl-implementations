...
Func blurX(Func continuation)
{
  ...
  blur_x(x, y, c) = (input_16(x-1, y, c) +
                     2 * input_16(x, y, c) +
                     input_16(x+1, y, c)) / 4;
  blur_x.vectorize(x, get_target_from_environment().natural_vector_size<uint8_t>()).parallel(y);
  blur_x.compute_root();
...
Func blurY(Func continuation)
{
  ...
  blur_y(x, y, c) = (input_16(x, y-1, c) +
                     2 * input_16(x, y, c) +
                     input_16(x, y+1, c)) / 4;
  blur_y.vectorize(y, get_target_from_environment().natural_vector_size<uint8_t>()).parallel(x);
  blur_y.compute_root();
